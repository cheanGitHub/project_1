...
public class QueneTests2 {

    private static final Logger LOGGER = LoggerFactory.getLogger(QueneTests2.class);
    ...
    public static void main(String[] args) throws InterruptedException {
        BlockingQueue<Runnable> blockingQueue = new ArrayBlockingQueue<>(3, true); // new LinkedBlockingQueue<>(3);
...
//        }).start();
        ...
        new Thread(() -> {
            while (true) {
                try {
                    ...
                    blockingQueue.drainTo(list); // drainTo();
                    if (!list.isEmpty()) {
        ...
        new Thread(() -> {
            try {
                for (int i = 1; i < 35; i++) {
                    int n = i;
                    Thread.sleep(RandomUtils.nextInt(1, 6) * 100);
